function [cPred, cTrue, tableOfEqn, tableOfErr, time] = weakIdentV18_VotingV5(Uhat, xs, SSInX, alphaBar, polys, Tau,  ...
    trueCoefficients, useCrossDerivative, SSInT, IC, useErrDyn, indOfInterestingFeatures_arr)
...
ScalesOfWandb            = mean(abs(SandbLarge),1);
L                        = size(WandbLarge,2);
...
b                        = WandbLarge(:,lhsInd);
cPreds = zeros(size(indOfInterestingFeatures_arr,1), size(WandbLarge,2)-length(lhsInd),numOfU);
for i = 1:size(indOfInterestingFeatures_arr,1)
    indOfInterestingFeatures = indOfInterestingFeatures_arr(i,:); % the reference feature
    [~, rm_vector] = findHydynamicRegionV3(dictList,indOfInterestingFeatures, SandbLarge, numOfBins);
    % ***
    idxHighlyDynamic         = []; % no highly dynamic
    % ***
    What = bsxfun(@times, rm_vector, W);
    bhat = bsxfun(@times, rm_vector, b);
    WTilda  = What .*scalesOfFeatures; % scaled
    bTilda  = bhat .*scaleOfb;
    if isempty(idxHighlyDynamic)% highly dynamic (not used)
        WNarrowTilda = WTilda;
        bNarrowTilda = bTilda;
    else
    ...
    for num_var = 1:numOfU
        if (i == 1)||(indOfInterestingFeatures(num_var)>0)
            bLarge_       = bhat(:,num_var);
            bLargeTilda_  = bTilda(:,num_var);
            bNarrowTilda_ = bNarrowTilda(:,num_var);
            scaleOfb_     = scaleOfb(num_var);
            supportPred   = weakIdentFeatureSelectionV2(What,bLarge_, ...
...
% cut off
numOfInterestingFeatures = 5;%size(indOfInterestingFeatures_arr,1);
threshold = floor(0.25*numOfInterestingFeatures);

cPred = zeros([size(WandbLarge,2)-length(lhsInd),numOfU]);
avg_magnitude = zeros([size(WandbLarge,2)-length(lhsInd),numOfU]);
for num_var = 1:numOfU
    cnt = squeeze(sum(cPreds(:,:,num_var)~=0, 1));
    avg_magnitude(:,num_var) = squeeze(sum(abs(cPreds(:,:,num_var)), 1));
    avg_magnitude(cnt<=threshold, num_var) = 0;
    avg_magnitude(cnt>threshold,num_var) = avg_magnitude(cnt>threshold,num_var) ./ numOfInterestingFeatures;% size(indOfInterestingFeatures_arr,1);
    
    % cut off
    supportPred = avg_magnitude(:,num_var) > 0.05*max(avg_magnitude(:,num_var), [], 'all' );
    
    % coeffs
    relativeScales = scalesOfFeatures' ./ scaleOfb(num_var);
    WTilda  = W .*scalesOfFeatures; % scaled
    bTilda  = b .*scaleOfb;
    cPred(supportPred,num_var) = WTilda(:,supportPred) \ bTilda(:,num_var) .* relativeScales(supportPred);
end
