classdef ModelComparer < handle
    % ModelComparer Class for handling model-related tasks like creating handler names,
    % output directories, and log files.
    %
    % Usage:
    %   handler = ModelComparer(config);
    %   handler.createOutputDir();
    %   handler.createLogFile();
    %
    % Properties:
    %   comparerName  - The name assigned to the handler, typically a model name with a timestamp.
    ...
    %
    % Methods:
    %   ModelComparer     - Constructor to initialize the handler with a config.
    ...
    properties (Access=public)
        ...
        comparerName % Stores the handler name (e.g., model name with dataset name)
    ...
    methods
        ...
        function self=ModelComparer(config)
            % ModelComparer Constructor to create a new handler object.
            %
            % Syntax:
        ...
        function createComparerName(self)
            self.comparerName = "ModelComparer";
        end
        function createOutputDir(self)
            self.outputDir = './Results/ModelComparer';
        ...
        function createLogFile(self)
            ...
            self.logFilePath = fullfile(self.outputDir, sprintf('Output_%s.txt', self.comparerName));
            % Open log file
